...
begin
  ...
  begin
    Integer(SectionHeader) := Integer(Pointer(@NtHeaders^.OptionalHeader)) +
                     Integer(NtHeaders^.FileHeader.SizeOfOptionalHeader) +
    ...
    then begin
      // long name
      Integer(p) :=  Integer(ModulePtr) +
            NTHeaders^.FileHeader.PointerToSymbolTable +
            NTHeaders^.FileHeader.NumberOfSymbols * IMAGE_SIZEOF_SYMBOL +
            StrToIntDef(PChar(@SectionName[1]), 0);
      Add(PChar(p), Pointer(Integer(ModulePtr) + SectionHeader^.PointerToRawData), SectionHeader^.Misc.VirtualSize,  SectionHeader^.VirtualAddress);
    end
    else begin
      // short name
      Add(SectionName, Pointer(Integer(ModulePtr) + SectionHeader^.PointerToRawData),
            SectionHeader^.Misc.VirtualSize,  SectionHeader^.VirtualAddress);
    end
  end;
end;
...
begin
  ...
  try
    ...
    AHeaders := Pointer(Integer(FModulePtr) + DosHeader^.e_lfanew);
    Result := True;
  finally
    if not Result
    then begin
